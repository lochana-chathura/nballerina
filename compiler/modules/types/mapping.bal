...
public function defineMappingTypeWrapped(MappingDefinition md, Env env, Field[] fields, SemType rest, CellMutability mut = CELL_MUT_LIMITED) returns SemType {
    ...
    CellField[] cellFields = from var { name, ty, ro, opt } in fields select [name, cellContaining(env, opt ? union(ty, UNDEF): ty, ro ? CELL_MUT_NONE : mut)];
...
function mappingInhabited(Context cx, TempMappingSubtype pos, Conjunction? negList) returns boolean {
    if negList == () {
        ...
        foreach var { name, index2, type1: posType, type2: negType } in pairing {
            if index2 == () {
                continue;
            }
            CellSemType d = <CellSemType>diff(posType, negType);
            if !isEmpty(cx, d) {
                TempMappingSubtype tmpNegType = { names:[name], types:[cellContaining(cx.env, union(complement(cellInner(negType)), UNDEF))], rest:cellContaining(cx.env, union(createAnydata(cx), UNDEF)) };
                TempMappingSubtype mt = intersectMapping(cx.env, pos, tmpNegType) ?: { names:[], types:[], rest: cellContaining(cx.env, UNDEF, CELL_MUT_NONE) };
