...
public function errorDetail(Context cx, SemType detail) returns SemType {
    SubtypeData sd = bddIntersect(<Bdd>subtypeData(detail, BT_MAPPING), MAPPING_RO_BDD);
    if sd is boolean {
        if sd {
            return ERROR;
         }
...
function errorSubtypeComplement(SubtypeData t) returns SubtypeData {
    ...
    return bddIntersect(bddComplement(<Bdd>t), MAPPING_RO_BDD);
}
...
function errorSubtypeIsEmpty(Context cx, SubtypeData t) returns boolean {
    ...
    b = bddPosMaybeEmpty(b) ? bddIntersect(b, MAPPING_RO_BDD) : b;
